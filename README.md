...
2. Copy the AMI to your account
   ...
   **Restore image**
    ...
    ```shell
    aws ec2 create-restore-image-task --object-key ami-0ec6fdfb365e5fc00.bin --bucket udacity-srend --name "udacity-project-2-sre-minh"
    ```
    ...
    - Take note of that AMI ID the script just output. Copy the AMI to `us-east-2` and `us-west-1`:
        - `aws ec2 copy-image --source-image-id ami-03daadac9e88fd231 --source-region us-east-1 --region us-east-2 --name "ima-p2-sre-minh"`
        - `aws ec2 copy-image --source-image-id ami-03daadac9e88fd231 --source-region us-east-1 --region us-west-1 --name "ima-p2-sre-minh"`

    - Make note of the ami output from the above 2 commands. You'll need to put this in the `ec2.tf` file for `./starter-code/zone1` for `us-east-2` and in `ec2.tf` file for `./starter-code/zone2` for `us-west-1` respectively

    <!-- - Set your aws cli config to `us-east-2` -->
    `aws s3api create-bucket --bucket project-2-sre-course-minh --region us-east-2 --create-bucket-configuration LocationConstraint=us-east-2`

3. Close your CloudShell. Change your region to `us-east-2`in AWS console. From the AWS console create an S3 bucket in `us-east-2` called `udacity-tf-<your_name>` e.g `udacity-tf-minh`
    - click next until created.
    - Update `_config.tf` in the `./starter-code/zone1` folder with your S3 bucket name where you will replace e.g `<your_name>` with your name
    - **NOTE**: S3 bucket names MUST be globally unique!

4. Change your region to `us-west-1`. From the AWS console create an S3 bucket in `us-west-1` called `udacity-tf-<your_name>-west` e.g `udacity-tf-minh-west`
    - click next until created.
    - Update `_config.tf` in the `./starter-code/zone2` folder with your S3 bucket name where you will replace `<your_name>` with your name
    - **NOTE**: S3 bucket names MUST be globally unique!
...
8. Setup Kubernetes config so you can ping the EKS cluster
   ...
   - Change kubernetes context to the new AWS cluster
     - `kubectl config use-context <cluster_name>`
       - e.g ` arn:aws:eks:us-east-2:685574950167:cluster/udacity-cluster`
